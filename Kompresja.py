...
def closest_centroids(X, centroids):
    '''
    Function computes distance (squared Euclidean distance) between pixels and centroids and find
...
def run_K_means (path, iterations, K, img_type):
    '''
    ...
    '''
    assert type(img_type) == str, "Please, make sure that you typed image type as string!"
    assert img_type == "png" or img_type == "jpeg" or img_type == "jpg", "Only png, jpeg, jpg formats are accepted!"
    
    
    X, original_picture, original_shape = load_image(path, img_type)
...
original_picture, recovered_picture = run_K_means("images/Kitty.jpg", 20, 15, "jpg")
